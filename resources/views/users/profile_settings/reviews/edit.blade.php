	...
	<div class="col-md-12 menu-container">
		...
		<div class="order-container">
			<div class="order">
				...
				@endphp
				<div>
					<img src="{{asset('storage/'.$rating->order->quantity->product->category->name.'/'.$images[0])}}" width="200px" height="200px" alt="">
				</div>
				<div class="product-review">
					<p>
						<div class="product-name">{{$rating->order->quantity->product->name}}</div>
						<div class="product-category">{{$rating->order->quantity->product->category->name}}</div>
						<div class="quantity">Qty: {{$rating->order->ordered_quantity}}</div>
						<div class="price">$ {{$rating->order->price}}</div><br>
		...
		<form action="/my/reviews/update/{{$rating->id}}" class="new-review-form" method="POST">
			<div class="input">
				...
				<div class='rating-stars'>
					...
					<input type="hidden" name="rating" id="rating" value="{{$rating->rating}}" required>
				</div>
			</div>
			<div class="input">
				<label for="feedback">Feedback:</label>
				<textarea name="feedback" id="feedback" class="form-control" cols="20" rows="5">{{$rating->feedback}}</textarea>
			</div>
			<div class="input">
				<p></p>
				<p>
					<input type="submit" value="Update Rating!" class="button" id="submit">
				</p>
			</div>
			@csrf
			@method('PUT')
